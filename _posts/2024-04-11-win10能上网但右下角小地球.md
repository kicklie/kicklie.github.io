...
作为解决方法，您可以尝试以下方法：  
确保此注册表项[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\NlaSvc\Parameters\Internet]启用主动探测，"EnableActiveProbing"设置为1。
确保此注册表项[HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\NetworkConnectivityStatusIndicator]启用被动探测，"NoActiveProbe"设置为 0（不应存在或值必须为0）。
