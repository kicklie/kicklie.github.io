...
首先，我们先要刷入临时 OP 固件。刷 OP 固件第一步是解锁 SSH，有关解锁 SSH 的操作步骤，请阅读**解锁 SSH** 部分。刷机前我们先要将临时 OP 固件`xiaomimtd12.bin`上传到路由器的 `/tmp` 文件夹。![](https://github.com/kicklie/kicklie.github.io/raw/master/img/ax6_01.jpg) 上传完毕后，SSH登录到路由器的后台，输入以下命令：
...
其中，数字 **0** 代表 **12** 分区。 这两行命令的含义为**下次启动时将 12 分区作为系统分区**，也就是从 **12** 分区启动。设置好了启动分区，输入命令 **mtd write /tmp/xiaomimtd12.bin rootfs**，将前面上传的临时 OP 固件刷入到 **12** 分区。完成后，输入 **reboot** 重启路由器。 ![](https://github.com/kicklie/kicklie.github.io/raw/master/img/ax6_02.jpg) 稍等片刻，在浏览器中输入临时 OP 固件的默认登录地址`192.168.1.1`。如果能正常打开登录页面，表示系统启动完毕。
...
下面我们将 OP 系统刷入到 **13 分区**就可以完成扩容刷机的操作了！<br>
首先，我们上传要刷入的 OP 固件`openwrt-ipq807x-generic-xiaomi_ax6-squashfs-nand-factory.bin`。![](https://github.com/kicklie/kicklie.github.io/raw/master/img/ax6_07.jpg) 上传完毕后，SSH登录到路由器的后台，输入命令 **ubiformat /dev/mtd13 -y -f /tmp/openwrt-ipq807x-generic-xiaomi_ax6-squashfs-nand-factory.bin**，将上传的 OP 固件刷入到 13 分区。完成后，我们输入下面三条命令，将启动分区切换为 13 分区后重启。
...
> 备注
> 1. `openwrt-ipq807x-generic-xiaomi_ax6-squashfs-nand-factory.bin`其实是QSDK的固件。刷完这个固件，打开浏览器登录到路由器，可以从网页端刷入`openwrt-ipq807x-generic-xiaomi_ax6-squashfs-nand-sysupgrade.bin`
> 2. 但是，如果要继续刷入LEDE或者其他固件，则不能直接从网页端刷入。原因就是QSDK固件有校验，无法通过web界面直接更新（会报错）。我们可以使用 sysupgrade 或 ubiformat 强制更新,下面以刷入LEDE的固件为例演示。
> **2.1** 使用sysupgrade（未验证）
> 使用的升级包：openwrt-ipq807x-generic-redmi_ax600-squashfs-nand-**sysupgrade**.bin
> 使用WinSCP将文件上传到路由器的`/tmp`目录下。
> 再用SSH登录到终端中，执行命令：
> `sysupgrade -F -n /tmp/文件名`
> 刷写后，会自动重启，等到网口自动连上即可。
> **2.2** 使用ubiformat（亲自验证，可行）
> 使用的升级包：openwrt-ipq807x-generic-redmi_ax6-squashfs-nand-**factory.ubi**
> 先SSH登录到终端中，使用如下命令切换到rootfs中：
> `fw_setenv flag_last_success 0`
> `fw_setenv flag_boot_rootfs 0`
> `reboot`
> **重启**路由器后，IP地址可能会变，注意鉴别。
> 用WinSCP将文件上传到路由器的`/tmp`目录下，运行如下命令进行写入：
> `ubiformat /dev/mtd13 -y -f /tmp/文件名`
> `fw_setenv flag_last_success 1`
> `fw_setenv flag_boot_rootfs 1`
> 文件名是最终的包名。完成后运行 reboot 重启即可。
> 3. 不管是QSDK还是LEDE固件，进入网页后删除不需要的软件包时都会报错。这个问题是由DDNS软件包引起的，所以我们要先把DDNS这个软件包给删除。如果直接删除DDNS软件包还是报错，那就重启路由器。登录后，先把DDNS的自动启动给禁用掉，再删除DDNS。之后删除其他软件包的时候就不会报错了。
