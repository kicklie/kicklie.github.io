...
> 2. 但是，如果要继续刷入LEDE或者其他固件，则不能直接从网页端刷入。原因就是QSDK固件有校验，无法通过web界面直接更新（会报错）。我们可以使用 sysupgrade 或 ubiformat 强制更新，下面以刷入LEDE的固件为例演示。<br>
...
> 先SSH登录到终端中，使用如下命令切换到rootfs中：<br>
> `fw_setenv flag_last_success 0`<br>
> `fw_setenv flag_boot_rootfs 0`<br>
> `reboot`<br>
> **重启**路由器后，IP地址可能会变，注意鉴别。<br>
> 用WinSCP将文件上传到路由器的`/tmp`目录下，运行如下命令进行写入：<br>
> `ubiformat /dev/mtd13 -y -f /tmp/文件名`<br>
> `fw_setenv flag_last_success 1`<br>
> `fw_setenv flag_boot_rootfs 1`<br>
> 文件名是最终的包名。完成后运行 reboot 重启即可。<br>
> 3. 不管是QSDK还是LEDE固件，进入网页后删除不需要的软件包时都会报错。这个问题是由DDNS软件包引起的，所以我们要先把DDNS这个软件包给删除。如果直接删除DDNS软件包还是报错，那就重启路由器。登录后，先把DDNS的自动启动给禁用掉，再删除DDNS。之后删除其他软件包的时候就不会报错了。<br>
